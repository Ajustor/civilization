...
// Coûts calibrés pour que l'arbre complet demande environ deux semaines réelles
// à une civilisation investie en mode rapide (12 mois par tick de 15 minutes),
// soit ~16 000 mois de jeu. Les multiplicateurs de savoir sont volontairement
// modestes : cumulés, ils plafonnent autour de ×5 (contre ×85 auparavant).
export const TECH_TREE: TechNode[] = [
  // ═══════════════════════════════════════════════════════════════
  // TIER 0 — Racines (aucun prérequis)
  // ═══════════════════════════════════════════════════════════════
  {
    id: TechId.CONSTRUCTION,
    name: 'Construction',
    description: 'Débloque la Maison, évolution de la Tente (chaque maison consomme une tente).',
    cost: 40,
    prerequisites: [],
    effects: [{ kind: 'unlockBuilding', building: BuildingTypes.HOUSE }],
  },
  {
  ...
  },
  {
    ...
    name: 'Entreposage',
    description:
      '+50 % de capacité de stockage. Débloque l\'Entrepôt, évolution de la Cache (chaque entrepôt consomme 2 caches, stocke 3× plus).',
    cost: 75,
    prerequisites: [],
    effects: [
      { kind: 'storageMultiplier', factor: 1.5 },
      { kind: 'unlockBuilding', building: BuildingTypes.WAREHOUSE },
    ],
  },
  {
    id: TechId.MEDICINE,
  ...
  },
  {
    id: TechId.WRITING,
    name: 'Écriture',
    description: '+10 % de production de savoir.',
    cost: 55,
    prerequisites: [],
    effects: [{ kind: 'researchMultiplier', factor: 1.10 }],
  },
  {
    id: TechId.FISHING,
    name: 'Pêche',
  ...
  },
  {
    id: TechId.THEOLOGY,
    name: 'Théologie',
    description: '+10 % de production de savoir et +5 % de chances de conception.',
    cost: 3800,
    prerequisites: [TechId.PHILOSOPHY, TechId.SCIENCES],
    effects: [
      { kind: 'researchMultiplier', factor: 1.10 },
      { kind: 'pregnancyProbabilityBonus', amount: 5 },
    ],
  },
  {
  ...
  },
  {
    ...
    name: 'Renaissance',
    description: '+15 % de production de savoir et +10 % de production.',
    cost: 4300,
    prerequisites: [TechId.ASTRONOMY, TechId.PRINTING],
    effects: [
      { kind: 'researchMultiplier', factor: 1.15 },
  ...
  },
  {
    ...
    description: '+2 enfants par femme et +10 % de production de savoir.',
    cost: 10500,
  ...
  // ═══════════════════════════════════════════════════════════════
  {
    ...
    name: 'Électricité',
    description: '+25 % de production des bâtiments et +15 % de production de savoir.',
    cost: 95000,
    prerequisites: [TechId.INDUSTRIALIZATION],
    effects: [
      { kind: 'productionMultiplier', factor: 1.25 },
      { kind: 'researchMultiplier', factor: 1.15 },
    ],
  },
  {
    id: TechId.MODERN_MEDICINE,
    name: 'Médecine Moderne',
    description: '+4 enfants par femme et +25 % de chances de conception.',
    cost: 100000,
    prerequisites: [TechId.PUBLIC_HEALTH, TechId.INDUSTRIAL_CHEMISTRY],
    effects: [
      { kind: 'maxChildrenBonus', amount: 4 },
      { kind: 'pregnancyProbabilityBonus', amount: 25 },
    ],
  },
  {
    id: TechId.GLOBALIZATION,
    name: 'Mondialisation',
    description: '+20 % de production et +50 % de capacité de stockage.',
    cost: 110000,
    prerequisites: [TechId.EMPIRE, TechId.ECONOMICS],
    effects: [
      { kind: 'productionMultiplier', factor: 1.20 },
      { kind: 'storageMultiplier', factor: 1.50 },
    ],
  },
]
