...
const MAXIMUM_INCOMING_PEOPLE_RATE = 25
const MAXIMUM_OUTGOING_PEOPLE_RATE = 10
...
export class Migration implements WorldEvent {
  ...
  actions({ civilizations }: Required<Pick<ActionInput, 'civilizations'>>): void {
    for (const civilization of civilizations) {
      ...
      const adults = civilization.getPeopleOlderThan(MINIMUM_CONCEPTION_AGE).toSorted(() => Math.random() - 0.5)
      ...
      const incomingPeopleCount = ~~(civilization.citizensCount * (INCOMING_PEOPLE_RATE / 100))
      const genders = [Gender.MALE, Gender.FEMALE]
      const occupations = [OccupationTypes.CARPENTER, OccupationTypes.FARMER]

      const incomingPeople = Array.from(
        { length: incomingPeopleCount },
        () => new PeopleBuilder()
